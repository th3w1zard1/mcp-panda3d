...
If you're using SmitheryAI to manage your MCP servers, follow these steps:

1. Create a `Dockerfile` in your repository root that defines how to build your MCP server. Here's an example Dockerfile for a Node-based MCP server:
...
2. Create a `smithery.yaml` file in your repository root. This file defines how your MCP server should be started. Here's an example:
...
3. Add your server to SmitheryAI:

   ```bash
   smithery add panda3d-docs-server
   ```

For more details, refer to the [SmitheryAI documentation](https://smithery.ai/docs/config).
